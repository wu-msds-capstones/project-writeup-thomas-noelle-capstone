...
results_heatmap_infra$Feature <- factor(results_heatmap_infra$Feature, levels = c(
  "Total Watts", "Speed Limit", "Sum Traffic Calming Devices", "Infrastructure Present", "Bike Lane"
))
```

## Modeling Results - All Crashes
```{r}
results_heatmap_all <- results_heatmap %>%
  filter(`Data Subset` == "All")
results_all_features <- results_heatmap_all %>%
  pull(Feature) %>%
  unique()
results_heatmap_all <- results_heatmap_all %>%
  mutate(Feature = factor(Feature, levels = levels(droplevels(results_all_features))))

results_heatmap_all_infra <- results_heatmap_infra %>%
  filter(`Data Subset` == "All")
results_all_features_infra <- results_heatmap_all_infra %>%
  pull(Feature) %>%
  unique()
results_heatmap_all_infra <- results_heatmap_all_infra %>%
  mutate(Feature = factor(Feature, levels = levels(results_all_features_infra)))
...
heatmap_all_all1 <- results_heatmap_all %>% 
  filter(`Target Variable` == "Injured vs Non-Injured") %>%
  mutate(Model_ID = factor(Model_ID, levels = inj_levels)) %>%
...
heatmap_all_all2 <- results_heatmap_all %>% 
  filter(`Target Variable` == "Degree of Injury") %>%
  mutate(Model_ID = factor(Model_ID, levels = deg_levels)) %>%
  ggplot(aes(x = Feature, y = fct_rev(Model_ID), fill = as.factor(Importance_Rank))) +
    geom_tile(color = "white") +
    scale_fill_manual(
      values = c("1" = "#1b9e77", "2" = "#d95f13", "3" = "#7570b3"),
...
heatmap_all_all3 <- results_heatmap_all %>%
  filter(`Target Variable` == "Fatal vs Non-Fatal") %>%
  mutate(Model_ID = factor(Model_ID, levels = ftl_levels)) %>%
  ggplot(aes(x = Feature, y = fct_rev(Model_ID), fill = as.factor(Importance_Rank))) +
    geom_tile(color = "white") +
    scale_fill_manual(
      values = c("1" = "#1b9e77", "2" = "#d95f13", "3" = "#7570b3"),
...
heatmap_all_inf1 <- results_heatmap_all_infra %>% 
  filter(`Target Variable` == "Injured vs Non-Injured") %>%
  mutate(Model_ID = factor(Model_ID, levels = inj_levels)) %>%
  ggplot(aes(x = Feature, y = fct_rev(Model_ID), fill = as.factor(Importance_Rank))) +
    geom_tile(color = "white") +
...
heatmap_all_inf2 <- results_heatmap_all_infra %>% 
  filter(`Target Variable` == "Degree of Injury") %>%
  mutate(Model_ID = factor(Model_ID, levels = deg_levels)) %>%
  ggplot(aes(x = Feature, y = fct_rev(Model_ID), fill = as.factor(Importance_Rank))) +
    geom_tile(color = "white") +
...
heatmap_all_inf3 <- results_heatmap_all_infra %>%
  filter(`Target Variable` == "Fatal vs Non-Fatal") %>%
  mutate(Model_ID = factor(Model_ID, levels = ftl_levels)) %>%
  ggplot(aes(x = Feature, y = fct_rev(Model_ID), fill = as.factor(Importance_Rank))) +
    geom_tile(color = "white") +
...
heatmap_all_visual <- (
  heatmap_all_all1 + plot_spacer() + heatmap_all_inf1 +
  plot_spacer() + plot_spacer() + plot_spacer() +
  heatmap_all_all2 + plot_spacer() + heatmap_all_inf2 +
  plot_spacer() + plot_spacer() + plot_spacer() +
  heatmap_all_all3 + plot_spacer() + heatmap_all_inf3
) +
  plot_layout(
    ncol = 3,
    widths = c(2, -0.4, 1),
    heights = c(0.5, -0.51, 0.5, -0.51, 0.5)
  )
```
...
```{r}
results_heatmap_bike <- results_heatmap %>%
  filter(`Data Subset` == "Bike")
results_bike_features <- results_heatmap_bike %>%
  pull(Feature) %>%
  unique()
results_heatmap_bike <- results_heatmap_bike %>%
  mutate(Feature = factor(Feature, levels = levels(droplevels(results_bike_features))))

results_heatmap_bike_infra <- results_heatmap_infra %>%
  filter(`Data Subset` == "Bike")
results_bike_features_infra <- results_heatmap_bike_infra %>%
  pull(Feature) %>%
  unique()
results_heatmap_bike_infra <- results_heatmap_bike_infra %>%
  mutate(Feature = factor(Feature, levels = levels(results_bike_features_infra)))

heatmap_bike_all1 <- results_heatmap_bike %>% 
  filter(`Target Variable` == "Injured vs Non-Injured") %>%
  mutate(Model_ID = factor(Model_ID, levels = inj_levels)) %>%
  ggplot(aes(x = Feature, y = fct_rev(Model_ID), fill = as.factor(Importance_Rank))) +
    geom_tile(color = "white") +
...
heatmap_bike_all2 <- results_heatmap_bike %>% 
  filter(`Target Variable` == "Degree of Injury") %>%
  mutate(Model_ID = factor(Model_ID, levels = deg_levels)) %>%
  ggplot(aes(x = Feature, y = fct_rev(Model_ID), fill = as.factor(Importance_Rank))) +
    geom_tile(color = "white") +
...
heatmap_bike_all3 <- results_heatmap_bike %>%
  filter(`Target Variable` == "Fatal vs Non-Fatal") %>%
  mutate(Model_ID = factor(Model_ID, levels = ftl_levels)) %>%
  ggplot(aes(x = Feature, y = fct_rev(Model_ID), fill = as.factor(Importance_Rank))) +
    geom_tile(color = "white") +
...
heatmap_bike_inf1 <- results_heatmap_bike_infra %>% 
  filter(`Target Variable` == "Injured vs Non-Injured") %>%
  mutate(Model_ID = factor(Model_ID, levels = inj_levels)) %>%
  ggplot(aes(x = Feature, y = fct_rev(Model_ID), fill = as.factor(Importance_Rank))) +
    geom_tile(color = "white") +
...
heatmap_bike_inf2 <- results_heatmap_bike_infra %>% 
  filter(`Target Variable` == "Degree of Injury") %>%
  mutate(Model_ID = factor(Model_ID, levels = deg_levels)) %>%
  ggplot(aes(x = Feature, y = fct_rev(Model_ID), fill = as.factor(Importance_Rank))) +
    geom_tile(color = "white") +
...
heatmap_bike_inf3 <- results_heatmap_bike_infra %>%
  filter(`Target Variable` == "Fatal vs Non-Fatal") %>%
  mutate(Model_ID = factor(Model_ID, levels = ftl_levels)) %>%
  ggplot(aes(x = Feature, y = fct_rev(Model_ID), fill = as.factor(Importance_Rank))) +
    geom_tile(color = "white") +
...
heatmap_bike_visual <- (
  heatmap_bike_all1 + plot_spacer() + heatmap_bike_inf1 +
  plot_spacer() + plot_spacer() + plot_spacer() +
  heatmap_bike_all2 + plot_spacer() + heatmap_bike_inf2 +
  plot_spacer() + plot_spacer() + plot_spacer() +
  heatmap_bike_all3 + plot_spacer() + heatmap_bike_inf3
) +
  plot_layout(
    ncol = 3,
    widths = c(2.2, -0.4, 0.8),
    heights = c(0.5, -0.51, 0.5, -0.51, 0.5)
  )
```
...
```{r}
results_heatmap_ped <- results_heatmap %>%
  filter(`Data Subset` == "Ped")
results_ped_features <- results_heatmap_ped %>%
  pull(Feature) %>%
  unique()
results_heatmap_ped <- results_heatmap_ped %>%
  mutate(Feature = factor(Feature, levels = levels(droplevels(results_ped_features))))

results_heatmap_ped_infra <- results_heatmap_infra %>%
  filter(`Data Subset` == "Ped")
results_ped_features_infra <- results_heatmap_ped_infra %>%
  pull(Feature) %>%
  unique()
results_heatmap_ped_infra <- results_heatmap_ped_infra %>%
  mutate(Feature = factor(Feature, levels = levels(results_ped_features_infra)))

heatmap_ped_all1 <- results_heatmap_ped %>% 
  filter(`Target Variable` == "Injured vs Non-Injured") %>%
  mutate(Model_ID = factor(Model_ID, levels = inj_levels)) %>%
  ggplot(aes(x = Feature, y = fct_rev(Model_ID), fill = as.factor(Importance_Rank))) +
    geom_tile(color = "white") +
...
heatmap_ped_all2 <- results_heatmap_ped %>% 
  filter(`Target Variable` == "Degree of Injury") %>%
  mutate(Model_ID = factor(Model_ID, levels = deg_levels)) %>%
  ggplot(aes(x = Feature, y = fct_rev(Model_ID), fill = as.factor(Importance_Rank))) +
    geom_tile(color = "white") +
...
heatmap_ped_all3 <- results_heatmap_ped %>%
  filter(`Target Variable` == "Fatal vs Non-Fatal") %>%
  mutate(Model_ID = factor(Model_ID, levels = ftl_levels)) %>%
  ggplot(aes(x = Feature, y = fct_rev(Model_ID), fill = as.factor(Importance_Rank))) +
    geom_tile(color = "white") +
...
heatmap_ped_inf1 <- results_heatmap_ped_infra %>% 
  filter(`Target Variable` == "Injured vs Non-Injured") %>%
  mutate(Model_ID = factor(Model_ID, levels = inj_levels)) %>%
  ggplot(aes(x = Feature, y = fct_rev(Model_ID), fill = as.factor(Importance_Rank))) +
    geom_tile(color = "white") +
...
heatmap_ped_inf2 <- results_heatmap_ped_infra %>% 
  filter(`Target Variable` == "Degree of Injury") %>%
  mutate(Model_ID = factor(Model_ID, levels = deg_levels)) %>%
  ggplot(aes(x = Feature, y = fct_rev(Model_ID), fill = as.factor(Importance_Rank))) +
    geom_tile(color = "white") +
...
heatmap_ped_inf3 <- results_heatmap_ped_infra %>%
  filter(`Target Variable` == "Fatal vs Non-Fatal") %>%
  mutate(Model_ID = factor(Model_ID, levels = ftl_levels)) %>%
  ggplot(aes(x = Feature, y = fct_rev(Model_ID), fill = as.factor(Importance_Rank))) +
    geom_tile(color = "white") +
...
heatmap_ped_visual <- (
  heatmap_ped_all1 + plot_spacer() + heatmap_ped_inf1 +
  plot_spacer() + plot_spacer() + plot_spacer() +
  heatmap_ped_all2 + plot_spacer() + heatmap_ped_inf2 +
  plot_spacer() + plot_spacer() + plot_spacer() +
  heatmap_ped_all3 + plot_spacer() + heatmap_ped_inf3
) +
  plot_layout(
    ncol = 3,
    widths = c(2.17, -0.4, 0.83),
    heights = c(0.5, -0.575, 0.5, -0.575, 0.5)
  )
```
