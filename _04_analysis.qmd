...
![**Figure 7:** Injury severity distribution comparing all crashes with bicycle and pedestrian-involved crashes.](figures/severity-dist.png)  
...
```{r}
#| label: figure-12
#| fig-cap: "**Figure 12:** Proportion of Road User Crash Outcome by Type"
#| fig-align: center
crashes_predictors %>% 
  ...
  mutate(`Road User Type` = factor(case_when(
    bike_flg == "Yes" ~ "Bike",
    ped_flg == "Yes" ~ "Pedestrian",
    TRUE ~ "Motorist"
  ), levels = c("Motorist", "Bike", "Pedestrian"))) %>%
  ...
  ggplot(aes(x = `Road User Type`, y = proportion, fill = Target_Flag)) +
    ...
    scale_fill_manual(values = c("#4682b4", "#228b22")) +
    geom_text(
...
all_predictors <- data.frame(
  ...
  Description = c(
    "Flag indicating whether an active participant in the crash had been using alcohol.",
    "Flag indicating whether an active participant in the crash had used drugs.",
    "Flag indicating whether an active participant in the crash had been using marijuana.",
    "Flag indicating whether or not a driver involved in the crash was exceeding the posted speed, driving too fast for conditions, or speed racing.",
    "Flag indicating whether the vehicle operator fled the scene of the crash.",
    "category indicating the way in which two or more roads meet or cross.",
    "Category indicating the condition of the travel lanes at the time of the crash.",
    "Category indicating  the alignment, contour, structure, or other distinctive feature that describes the
roadway at the crash location.",
    "Category indicating the atmospheric conditions at the time of the crash.",
    "Category indicating the amount of ambient light available at the time of the crash.",
    "Integer indicating the sum of power generated in watts by all surrounding street lights in a given intersection.",
    "Integer indicating the posted closest speed limit to the crash intersection.",
    "Integer indicating the sum of roadway traffic calming features (speed bumps, raised crosswalks, rumble strips, and textured crosswalks).",
    "Flag indicating whether an infrastructural feature is present (Neighborhood Greenway, other types of bike lanes, Enhanced Shared Roadways).",
    "Flag indicating whether a bike lane was present.",
    "Integer indicating the hour of the crash (Initial modeling showed negligible performance differences between numeric coding and categorical binning).",
    "Integer indicating the day of the week of the crash (Initial modeling showed negligible performance differences between numeric and categorical coding)."
  )
)
inf_predictors <- data.frame(
  Predictor = c(
    "Total Watts",
    "Speed Limit",
    "Sum Traffic Calming Devices",
...
```{r}
#| label: table-1
#| fig-cap: "**Table 1:** Description of all predictors used in models."
#| fig-align: center
tagList(
  tags$style(HTML("
    table.dataTable {
...
```{r}
#| label: table-2
#| fig-cap: "**Table 2:** Description of infrastructural predictors used in models."
#| fig-align: center
tagList(
  tags$style(HTML("
    table.dataTable {
